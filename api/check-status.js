...
const pool = createPool({
  connectionString: process.env.IPTV_PRO_POSTGRES_URL,
});
...
export default async function handler(request, response) {
  ...
  try {
    // Check if the device exists in the users table using the custom pool
    const { rows } = await pool.sql`SELECT device_id, trial_start_date, is_paid FROM users WHERE device_id = ${deviceId};`;
